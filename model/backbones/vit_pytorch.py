...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):    #dim=768
        super().__init__()
        ...
        self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)   #768 to 768*3
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = nn.Linear(dim, dim)
        self.proj_drop = nn.Dropout(proj_drop)

    def forward(self, x):
        B, N, C = x.shape   #1, 193, 768
...
class TransReID(nn.Module): #
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        if hybrid_backbone is not None:
            ...
            self.patch_embed = PatchEmbed_overlap(
                img_size=img_size, patch_size=[16, 16], stride_size=[16, 16], in_chans=in_chans,
                # img_size=img_size, patch_size=[4, 4], stride_size=stride_size, in_chans=in_chans,
                embed_dim=embed_dim)
    ...
    def forward(self, x, cam_label=None, view_label=None):
        ...
        n, p, c = x_patch.shape
        x_patch = torch.reshape(x_patch, [n, 24, 8, c])
    ...
    def load_param(self, model_path):
        ...
        for k, v in param_dict.items():
            ...
            if 'patch_embed.proj.weight' in k and len(v.shape) < 4:
                ...
                v = v.reshape(O, -1, H, W)
            elif k == 'pos_embed' and v.shape != self.pos_embed.shape:  #self.pos_embed.shape:1,129,768
...
def vit_base_patch16_224_COFDViT(img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, camera=8, view=0, local_feature=False, sie_xishu=1.5, **kwargs):
    model = TransReID(
